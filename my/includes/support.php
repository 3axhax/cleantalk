...
class Support extends CCS {
    ...
    protected function articles_json() {
        ...
        if (isset($_GET['q']) && !empty($_GET['q'])) {
            ...
            foreach ($keys as $k => $v) {
                $keys[$k] = sprintf("keyword LIKE '%%%s%%'", trim($v));
            }

            $keys = implode(' OR ', $keys);

            $rows = $this->db->select(sprintf("SELECT keyword_id FROM articles_keywords WHERE %s", $keys), true);

            if (!empty($rows)) {
                $keyword_id = [];
                foreach ($rows as $row)
                {
                    $keyword_id[] = sprintf("keywords REGEXP '(^|, )%d($|,)'", $row['keyword_id']);
                }
                $keyword_id = array_slice($keyword_id, 0, 3, true);
                $keyword_id = implode(' OR ', $keyword_id);
                if ($keyword_id != '') {
                    $sql = sprintf("SELECT a.article_id, a.article_title, l.seo_url FROM articles a LEFT JOIN links l ON l.id = a.article_linkid WHERE (%s) AND a.article_id NOT IN (1, 33) LIMIT 0,5", $keyword_id);
        ...
        if (empty($result)) $result['empty'] = 1;
